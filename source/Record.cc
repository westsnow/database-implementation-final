...
int Record :: ComposeRecord (Schema *mySchema, const char *src) {
	...
	if (space == NULL)
	{
		cout << "compose Record ERROR : Not enough memory. EXIT !!!\n";
		exit(1);
	}
	...
	if (recSpace == NULL)
	{
		cout << "recspace Record ERROR : Not enough memory. EXIT !!!\n";
		exit(1);
	}
	...
	if (bits == NULL)
	{
		cout << "bits Record ERROR : Not enough memory. EXIT !!!\n";
		exit(1);
	}
...
int Record :: SuckNextRecord (Schema *mySchema, FILE *textFile) {
	...
	if (space == NULL)
	{
		cout << "space Record ERROR : Not enough memory. EXIT !!!\n";
		exit(1);
	}
	...
	if (recSpace == NULL)
	{
		cout << "recspace 2 Record ERROR : Not enough memory. EXIT !!!\n";
		exit(1);
	}
	...
	if (bits == NULL)
	{
		cout << "bits 2 Record ERROR : Not enough memory. EXIT !!!\n";
		exit(1);
	}
...
void Record :: CopyBits(char *bits, int b_len) {
	...
	if (this->bits == NULL)
	{
		cout << "this bits Record ERROR : Not enough memory. EXIT !!!\n";
		exit(1);
	}
...
void Record :: Copy (Record *copyMe) {
	...
	if (bits == NULL)
	{
		cout << "bits 3 Record ERROR : Not enough memory. EXIT !!!\n";
		exit(1);
	}
...
void Record :: Project (int *attsToKeep, int numAttsToKeep, int numAttsNow) {
	...
	// cout<<"numAttsNow "<<numAttsNow<<endl;
	...
	if (newBits == NULL)
	{
		cout << "new bits Record ERROR : Not enough memory. EXIT !!!\n";
		exit(1);
	}
...
void Record :: MergeRecords (Record *left, Record *right, int numAttsLeft, int numAttsRight, int *attsToKeep, int numAttsToKeep, int startOfRight) {
	...
	if (bits == NULL)
	{
		cout << "bits 4 Record ERROR : Not enough memory. EXIT !!!\n";
		exit(1);
	}
